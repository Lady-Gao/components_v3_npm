<template>
  <div class="cv-tree" ref="tree" v-loading="treeLoading">
    <div class="cv-tree-emptytext" v-show="isEmptyText">
      {{ $t("components.common.empty") }}
    </div>
...
export default {
  ...
  methods: {
    ...
    rightCLickEvent(event, treeId, treeNode) {
      ...
      if (target && this.isContextmenu) {
        ...
        const { clientX, clientY } = event,
          { top, left } = this._tree.getBoundingClientRect();
    ...
    _asyncLoadChilds(parentNode, name = "check") {
      ...
      const result = this.nodeFilter[1]
        ? this.nodeFilter[1]
        : name == "check"
        ? true
        : false;
    ...
    handlerCheck(event, treeId, treeNode) {
      ...
      if (!zAsync) {
        ...
        if (isParent) {
          ...
          let checkedChild =
            treeNode[condition] == result || treeNode[condition] == 5
              ? [treeNode]
              : [];
          // check勾选变化的回调函数
          this.currentChange({
    ...
    handlerClick(treeNode) {
      ...
      const condition = this.nodeFilter[0] ? this.nodeFilter[0] : "checked";
      const result = this.nodeFilter[1] ? this.nodeFilter[1] : true;
      var emitObj = treeNode;
      if (this.allowCancel) {//反选需求
        treeNode.checked = true;
        if (this.lastTid == treeNode.tId) {
          treeNode.checked = false;
          emitObj = {};
          this.lastTid = null;
          console.log(treeNode.checked, "treeNode.checked=false");
        } else {
          this.lastTid = treeNode.tId;
        }
      }
      this.$emit("node-click", emitObj);
    ...
    cancelSelectedNode(treeNode) {
      const { zTree } = this.tree;
      return zTree && zTree.cancelSelectedNode(treeNode);
    },
    ...
    filterNodes(value) {
      ...
      zTree.expandNode(childs[0], true, false, false);
      zTree.expandNode(parents[0], true, false, false);
    ...
    filterNodes_search(value, level = 1) {
      var parents = [];
      var { zTree } = this.tree;
      var results = zTree.getNodesByParamFuzzy(this.names, value);
    ...
    filterNodes_highlight(value, allNodes) {
      var { zTree } = this.tree;
      var all_nodes = zTree.transformToArray(
        allNodes ? allNodes : zTree.getNodes()
      );
  ...
  mounted() {
    ...
    this.tree = new BaseTree({
      ...
      methods: {
        ...
        nodeBeforebeforeRename(treeId, treeNode) {
          _this.$emit("node-before-rename", treeNode);
        },
        ...
        // 数据成功渲染完成的回调
        treeLoaded(data) {
          typeof _this.treeLoaded === "function" &&
            _this.treeLoaded(data, _this.tree.zTree);
        },
...
.cv-ztree li span.button.department_ico_open:before,
.cv-ztree li span.button.department_ico_close:before,
.cv-ztree li span.button.department_ico_docu:before {
  content: "\e639";
  @include font-family;
}
// 车组icon
